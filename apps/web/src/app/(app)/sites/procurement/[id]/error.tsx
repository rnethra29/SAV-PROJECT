...
type ProcurementOrderDetailErrorProps = {
  error: Error & { digest?: string };
  reset: () => void;
};

// Next.js route-segment error boundary — see sites/procurement/error.tsx
// for why this is defense-in-depth (the real GET /procurement-orders/:id
// call runs inside ProcurementOrderDetailContainer and handles its own
// loading/error/404/401 states without throwing).
export default function ProcurementOrderDetailError({ error, reset }: ProcurementOrderDetailErrorProps) {
  useEffect(() => {
    console.error(error);
  }, [error]);
