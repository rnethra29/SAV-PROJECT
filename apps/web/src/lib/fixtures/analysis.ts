...
import { getLatestMarketPrice } from "@/lib/fixtures/market-price";
import { getActualPrice } from "@/lib/fixtures/actual-price";
import { getCurrentQuotation, getQuotationItemsByRfqItemId } from "@/lib/fixtures/quotation";
import { getFinalOffer } from "@/lib/fixtures/negotiation";
import { getCurrentBoq, getBoqItems } from "@/lib/fixtures/boq";
import { getPoItemsByBoqItemId } from "@/lib/fixtures/po";
...
async function resolveRfqIdForItem(rfqItemId: string): Promise<string | null> {
  const { getRfqList } = await import("@/lib/fixtures/rfq");
