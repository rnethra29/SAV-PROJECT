...
/**
 * Inline upload form for the Documents section — real POST /documents
 * (multipart/form-data) via the caller's onSubmit, matching the backend's
 * actual required fields (src/validators/document.validator.js
 * #uploadDocument: entityType, entityId, documentCategoryId; file itself
 * via multer's `file` field, checked server-side in
 * document.controller.js#upload). No fake success — errors thrown by
 * onSubmit are shown inline and the form stays open so the user can retry.
 */
export function ClientDocumentUploadForm({ categories, onSubmit, onCancel }: ClientDocumentUploadFormProps) {
    ...
    <form onSubmit={handleSubmit} noValidate className="space-y-4 border-b border-border bg-background/40 p-5">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div className="space-y-1.5">
          <label htmlFor="document-file" className="block text-sm font-medium text-text-primary">
            File
          </label>
          <input
            id="document-file"
