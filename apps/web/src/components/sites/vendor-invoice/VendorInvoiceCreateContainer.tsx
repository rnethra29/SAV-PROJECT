...
import { VendorInvoiceCreateForm } from "./VendorInvoiceCreateForm";
...
type LoadState =
  | { kind: "loading" }
  | { kind: "success"; vendors: VndVendor[]; projects: ClmProjectLookup[]; purchaseOrders: VndPurchaseOrder[] }
  | { kind: "error"; message: string };

export function VendorInvoiceCreateContainer() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [{ vendors }, projects, { orders }] = DEV_FIXTURE_MODE
        ? await Promise.all([devGetVendorList(), devGetProjectOptions(), devGetProcurementOrderList()])
        : await Promise.all([getVendorList(), getProjectOptions(), getProcurementOrderList()]);
      setState({ kind: "success", vendors, projects, purchaseOrders: orders });
    } catch (error) {
      ...
      if (error instanceof ApiError && error.status === 403) {
        setState({
          kind: "error",
          message: "Your account doesn't have permission to create vendor invoices. Contact an administrator.",
        });
        return;
      }
      setState({
  ...
  return <VendorInvoiceCreateForm vendors={state.vendors} projects={state.projects} purchaseOrders={state.purchaseOrders} />;
}
