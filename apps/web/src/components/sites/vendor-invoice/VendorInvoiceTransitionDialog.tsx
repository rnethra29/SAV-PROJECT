...
export type VendorInvoiceTransition = {
  actionLabel: string;
  dialogTitle: string;
  description: string;
  tone: "primary" | "danger";
  fromStatus: VndInvoiceStatus;
  toStatus: VndInvoiceStatus;
  run: () => Promise<void>;
};

type VendorInvoiceTransitionDialogProps = {
  transition: VendorInvoiceTransition;
  onClose: () => void;
  onSuccess: () => void;
};

/**
 * Generic one-click invoice status confirmation dialog — same shape as
 * ProcurementOrderTransitionDialog/Commercial's RfqStatusTransitionDialog
 * (badge-to-badge preview, confirm button).
 */
export function VendorInvoiceTransitionDialog({ transition, onClose, onSuccess }: VendorInvoiceTransitionDialogProps) {
  ...
  return (
    <div role="presentation" onClick={onClose} className="fixed inset-0 z-50 flex items-center justify-center bg-text-primary/40 p-4">
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="invoice-transition-title"
        onClick={(e) => e.stopPropagation()}
        className="w-full max-w-md rounded-xl border border-border bg-surface p-6 shadow-xl"
      >
        <div className="flex items-start justify-between gap-4">
          <h2 id="invoice-transition-title" className="text-base font-semibold text-text-primary">
