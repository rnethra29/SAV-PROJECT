...
import { VendorPaymentCreateForm } from "./VendorPaymentCreateForm";
...
export function VendorPaymentCreateContainer() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (error instanceof ApiError && error.status === 403) {
        setState({
          kind: "error",
          message: "Your account doesn't have permission to create vendor payments. Contact an administrator.",
        });
        return;
      }
      setState({
  ...
  return <VendorPaymentCreateForm vendors={state.vendors} projects={state.projects} />;
}
