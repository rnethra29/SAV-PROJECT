...
Reference: [`01-commercial-lifecycle/ARCHITECTURE.md`](../../../01-commercial-lifecycle/ARCHITECTURE.md) — every endpoint below implements a section of that spec.
...
Gated transitions (`shared/backend/models/approvalStages.js`) — each requires a matching **Approved** `com_approvals` row first, or the request 409s with `code: APPROVAL_REQUIRED`:
...
Reference: [`02-site/01-client-management/ARCHITECTURE.md`](../../../02-site/01-client-management/ARCHITECTURE.md). Owns `clm_*` tables only; the RFQ->PO chain, Documents, Approvals and Audit Log above are **reused by FK / `entity_type` value**, not redefined (doc §2).
...
Reference: [`02-site/02-vendor-management/ARCHITECTURE.md`](../../../02-site/02-vendor-management/ARCHITECTURE.md). Owns `clm_project*`/`vnd_*`/`sec_*` only; RFQ→PO, Client Management, Documents, Approvals and Audit Log above are **reused by FK / `entity_type` value**, not redefined (doc §0/§2).
...
Gated transitions added by this submodule (`shared/backend/models/approvalStages.js`):
